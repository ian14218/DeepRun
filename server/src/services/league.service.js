...
async function updateLeague(id, fields, userId) {
  ...
  const allowed = ['name', 'team_count', 'roster_size'];
  const safeFields = {};
...
async function fillWithBots(leagueId, userId) {
  ...
  const existingBots = await userModel.findBotUsers(openSlots);
  ...
  for (let i = 0; i < openSlots; i++) {
    if (i < availableBots.length) {
      ...
      const botNum = memberCount + i + 1;
      const bot = await userModel.createBotUser(`CPU Bot ${botNum}`);
      botsToAdd.push(bot);
    }
  }
...
module.exports = { createLeague, joinLeague, getLeagueById, getLeaguesByUser, updateLeague, fillWithBots };
